...
| REQ-003 | Sensor faults (dropout, out-of-range, stale timestamp) shall not crash the fusion loop | `sensor.hpp`, `fusion.cpp` | `test_fusion.cpp::FaultInjectionTest_*`, `test_fusion_properties.cpp::SurvivesAnyFaultInterleaving` (randomized fault sequences via RapidCheck) |
...
| REQ-005 | Flight recorder entries shall be checksummed and detect corruption on replay | `flight_recorder.cpp` | `test_flight_recorder.cpp::CorruptionDetectionTest`, `fuzz/fuzz_flight_recorder.cpp` (libFuzzer on `decode_and_verify`, `fuzz-smoke-test` CI job) |
