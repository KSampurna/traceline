...
| M5 | Replay tool (CLI to visualize/replay recorded flights), polish docs — **done**: `traceline_replay` (`tools/`) replays a `FlightRecorder` log to human-readable text or CSV via the existing public `replay()` API, stopping with a named error on the first corrupted/truncated record; its formatting logic is unit-tested independently of `argv`/file I/O; README status, limitations, and stale CI forward-references corrected to match actual project state |
